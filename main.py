...
DOWNLOADED_FILE = BASE_DIR / _config.get("BOOK_DOWNLOADED_FILE", "downloaded_books.json")
...
def archive_book(url: str):
    """Move book from queue to downloaded."""
    book = get_book_by_url(url)
    if not book:
        raise ValueError("Book not found in queue")

    # Remove from queue
    remove_book_from_queue(url)

    # Add to downloaded
    downloaded = downloaded_books()
    if not isinstance(downloaded, dict):
        downloaded = {"books": []}
    if "books" not in downloaded:
        downloaded["books"] = []

    # Avoid duplicates
    if not any(b["book_url"] == url for b in downloaded["books"]):
        downloaded["books"].append(book)
...
@app.get("/api/downloaded/books")
def get_downloaded_books():
    """Return list of downloaded books."""
    return {"books": downloaded_books()}
...
@app.post("/api/books/{slug}/archive")
def archive_book_endpoint(slug: str):
    """Move a book from queue to downloaded."""
    books = queue_books()
    book = next((b for b in books if unquote(slug_from_url(b["book_url"])) == slug), None)
    if not book:
        raise HTTPException(404, "Book not found")
    ...
    try:
        archive_book(book["book_url"])
        return {"ok": True, "message": f"Book archived: {book.get('title', slug)}"}
    except ValueError as e:
